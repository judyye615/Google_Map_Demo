...
public class BaseClass {
   ...
   @BeforeClass
   public void setupBrowser() throws FileNotFoundException, IOException, InterruptedException {
 
       System.out.println("LOG INFO: Before suite running...setting up report");
       ExtentHtmlReporter reporter = new ExtentHtmlReporter(
               new File(System.getProperty("user.dir") + "/reports/ETReport" + utility.getTime() + ".html"));
   ...
   public void appendReport(ITestResult result) throws IOException {
       ...
       if (status == ITestResult.SUCCESS) {
           ...
           logger.fail("Test Scenario Failed " + result.getThrowable().getMessage(),
                   MediaEntityBuilder.createScreenCaptureFromPath(utility.captureScreenshot(driver)).build());
       }
